export default function handler(req, res) {
  ...
  // Determine the base API URL (Vercel URL for production, localhost for development)
  const apiUrl = process.env.VERCEL_URL 
    ? `https://${process.env.VERCEL_URL}`
    : 'http://localhost:3000';
  
  // Embed the API URL into the widget code
  const script = `
window.VTON_API_URL = '${apiUrl}';
${getWidgetCode()}
`;
...
function getWidgetCode() {
  // This function contains the minified or concatenated client-side JS code.
  // The provided content is already escaped for use inside a JavaScript template literal.
  return `(function() {
  'use strict';
  const CONFIG = {
  ...
  function injectStyles() {
    if (document.getElementById('vton-widget-styles')) return;
    const styles = document.createElement('style');
    styles.id = 'vton-widget-styles';
    styles.textContent = \`.vton-container {
      max-width: 600px;
      margin: 0 auto;
  ...
  class VirtualTryOnWidget {
    constructor(containerId) {
      ...
      if (!this.container) {
        console.error(\`[VTON] Container \${containerId} not found\`);
        return;
      }
      this.state = {
        userImage: null,
    ...
    updateGenerateButton(containerId) {
      ...
      btn.disabled = !(this.state.userImage && this.state.clothImage);
    }
    ...
    async generateTryOn(containerId) {
      ...
      try {
        ...
        if (!response.ok) {
          ...
          if (data.errorType === 'quota_limit') {
            message = 'AI service quota limit reached. Please try again later.';
          } else if (message.includes('Timeout') || message.includes('overloaded') || message.includes('asleep')) {
            message = 'Processing timed out. The AI model may be overloaded or asleep. Please try again in a few minutes.';
          }
        ...
        this.showError(message, containerId);
      } finally {
    ...
    showResult(containerId) {
      const img = document.getElementById(\`vton-result-image-\${containerId}\`);
      img.src = \`data:image/jpeg;base64,\${this.state.result}\`;
      document.getElementById(\`vton-input-section-\${containerId}\`).style.display = 'none';
      ...
      this.showSuccess('Try-on completed!', containerId);
    }
    ...
    showError(message, containerId) {
      ...
      const cssClass = (message.includes('quota') || message.includes('timeout') || message.includes('unavailable') || message.includes('overloaded') || message.includes('asleep')) 
        ? 'vton-error vton-error-quota' 
        : 'vton-error';
  ...
  function init() {
    ...
    containers.forEach(c => {
      if (!c.dataset.initialized) {
        ...
        new VirtualTryOnWidget(c.id);
      }
    });
    const legacy = document.getElementById('vton');
    if (legacy && !legacy.dataset.initialized) {
      legacy.dataset.initialized = 'true';
  ...
  window.VirtualTryOn = { Widget: VirtualTryOnWidget };
})();
`;
}